...
from app.assets import RLUSD_HEX, normalize_currency_code
...
def get_validated_trustline(
    ...
    for line in response.result.get("lines", []):
        if (
            ...
            and normalize_currency_code(str(line.get("currency", ""))) == "RLUSD"
        ):
            return line
    return None
...
def get_validated_trustline_balance(
    ...
    trustline = get_validated_trustline(client, address, issuer)
...
def wait_for_trustline_balance_increase(
    ...
    *,
    ...
    while time.time() < deadline:
        balance = get_validated_trustline_balance(client, address, issuer)
        if balance >= starting_balance + increase:
            return balance
        time.sleep(1)
    raise AssertionError(
        f"Timed out waiting for RLUSD balance increase of {increase} on {address}"
    )
...
def ensure_rlusd_trustline(
    ...
    *,
    ...
    if trustline_limit_is_sufficient(client, wallet.classic_address, issuer, Decimal(limit_value)):
        return None
    ...
    _assert_validated_success(response, "RLUSD trustline transaction")
...
def reset_rlusd_trustline(
    ...
    trustline = get_validated_trustline(client, wallet.classic_address, issuer)
    if trustline is None:
        return None
    ...
    if balance != 0:
        raise RuntimeError(
            f"Cannot reset RLUSD trustline for {wallet.classic_address} while balance is {balance}"
        )
    if limit == 0:
        return None
    ...
    response = submit_and_wait(trust_set, client, wallet).result
    _assert_validated_success(response, "RLUSD trustline reset")
    return _response_tx_hash(response)
...
def wait_for_trustline_removal(
    ...
    *,
    ...
    deadline = time.time() + timeout_seconds
    while time.time() < deadline:
        if get_validated_trustline(client, address, issuer) is None:
            return True
        time.sleep(1)
    return False
...
def submit_validated_rlusd_payment(
    ...
    _assert_validated_success(response, "RLUSD transfer")
    return _response_tx_hash(response)
